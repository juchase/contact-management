...
import { addressDetail, adressUpdate } from "../../lib/api/addressApi";
import { userDetail } from "../../lib/api/UserApi";

export default function AdressEdit() {
  ...
  useEffectOnce(() => {
    fetchContact().then(() => console.log("Contact fetched succesfully"));
    fetchAddress().then(() => console.log("Address fetched succesfully"));
  });

  async function handleSubmit(e) {
    e.preventDefault();

    const response = await adressUpdate(token, id, {
      addressId,
      street,
      city,
      province,
    ...
    if (response.status === 200) {
      await alertSuccess("Adress updated successfully");
      navigate({
